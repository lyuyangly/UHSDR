...
#define SI5351_MIN_PLL			600000000			// Min PLL frequency, officially it is 600 Mhz, but 405 seems to work just fine.
...
static bool Si5351a_CalculateConfig(uint32_t frequency, Si5351a_Config_t* new_config, Si5351a_Config_t* cur_config)
{
	...
	if (divider_max < 8 && divider_max % 2)
	{
	...
	if (divider_min < 8 && divider_min % 2)
	{
		divider_min++;						// Ensure an even integer division ratio
	}

	if (divider_max <= SI5351_MAX_DIVIDER)
	{
...
// The limits assume a 4x johnson counter
